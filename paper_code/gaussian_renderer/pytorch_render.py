...
class GaussRenderer(nn.Module):
    ...
    def __init__(self, active_sh_degree=3, white_bkgd=False, **kwargs):
        super(GaussRenderer, self).__init__()
        self.active_sh_degree = active_sh_degree
        self.debug = False
        self.white_bkgd = white_bkgd
        self.pix_coord = torch.stack(torch.meshgrid(torch.arange(1440), torch.arange(2560), indexing='xy'), dim=-1).to('cuda')
    ...
    def render(self, camera, means2D, cov2d, color, opacity, depths):
        ...
        return {
            "render": self.render_color.permute(2,0,1),
    ...
    def forward(self, camera, means3D, opacity, scales, rotations, color, **kwargs):

        mean_ndc, mean_view, in_mask = projection_ndc(means3D, 
                viewmatrix=camera.world_view_transform.cuda(), 
                projmatrix=camera.projection_matrix.cuda())
        mean_ndc = mean_ndc[in_mask]
        mean_view = mean_view[in_mask]
        depths = mean_view[:,2]
        # print(depths.shape, mean_view.shape)
